class ExercisesController < ApplicationController
  ...
  def exercise2
    ...
    #   * left_outer_joinsを使うこと
     @shops = Shop.eager_load(foods: :order_foods).where(order_foods: { food_id: nil })
  end
  ...
  @address = Address.joins(:orders)
                    ...
                    .eager_load(:orders)
                    .select('addresses.*, COUNT(orders.customer_id) AS orders_count')
                    .first
  end
  ...
  def exercise4 
    ...
    @customer = Customer.eager_load(orders: { order_foods: :food })
                        .group('customers.id')
